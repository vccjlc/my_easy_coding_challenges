...
void Reverse(Tlista **l, Tlista **kursor, int ile) {
    if (ile <= 1) {
        // No need to reverse if ile is 0 or 1
    ...
    Tlista *prev = NULL;
    Tlista *next = NULL;
    Tlista *last = *kursor; // This will end up being the 'prekursor' after the reversal
    ...
    // Connect the previous part of the list with the new first node after reversal
    if (*kursor != *l) {
        Tlista *temp = *l;
        ...
        *l = prev; // If the reversal started at the head of the list
    }

    // Connect the last node of the reversed segment to the rest of the list
    last->next = current;

    // Set the cursor to the element immediately after the reversed segment
    *kursor = current;

    // Set the prekursor to the last node of the reversed segment (which was the original cursor)
    prekursor = last;
}
